...
if ( ! function_exists( 'understrap_posted_on' ) ) {
	...
	function understrap_posted_on() {
		...
		if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
			...
			<time class="updated" datetime="%3$s">| Actualización: %4$s </time>';
		}
		...
		if ( 0 === $author_id ) {
			...
			$byline = apply_filters(
				...
				sprintf(
					...
					$posted_on ? ucfirst(esc_html_x( 'by', 'post author', 'understrap' )) : ucfirst(esc_html_x( 'Posted by', 'post author', 'understrap' )),
...
/*
		...
		if ( $categories_list && understrap_categorized_blog() ) {
			printf( '<span class="cat-links badge text-bg-dark">' . esc_html__( '%s', 'understrap' ) . '</span>', $categories_list ); 
		}
*/
		
		foreach ( ( get_the_category() ) as $category ) { ?>
			<span class="cat-links badge text-bg-dark mb-3"><?php echo $category->cat_name . ' ';?></span>
  		<? }
	}
}
